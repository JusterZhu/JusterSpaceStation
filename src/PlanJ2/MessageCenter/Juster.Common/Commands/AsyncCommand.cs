...
namespace Juster.Common
{
    ...
    public class AsyncCommand<TResult,T> : CommandBase, INotifyPropertyChanged
    {
        #region fields
        private readonly Func<CancellationToken, Task<TResult>> _command;
        private readonly Func<object, CancellationToken, Task<TResult>> _parameterCommand;
        private readonly CancelAsyncCommand _cancelCommand;
        private NotifyTaskCompletion<TResult> _execution;
        #endregion
        ...
        {
            _cancelCommand.NotifyCommandStarting();
            Execution = new NotifyTaskCompletion<TResult>(_parameterCommand(parameter,_cancelCommand.Token));
            OnPropertyChanged();
            await Execution.TaskCompletion;
            _cancelCommand.NotifyCommandFinished();
            OnPropertyChanged();
